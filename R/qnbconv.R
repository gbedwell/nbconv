...
#'
#'@param probs Numeric vector of target (cumulative) probabilities.
#'@param counts Integer vector of counts over which the PMF is evaluated.
...
qnbconv <- function(probs, counts, mus, ps, phis, method = c("exact", "moments", "saddlepoint"),
  ...
  counts <- seq( 0, max(counts) )

  method <- match.arg( method )

  if( method != "exact" & method != "moments" & method != "saddlepoint" ){
    stop( "method must be one of 'exact', 'moments', or 'saddlepoint'.",
          call. = FALSE )
  }

  if(!missing(ps) & !missing(mus)){
    stop("Only one of mus or ps should be specified.", call. = FALSE)
  }

  if( missing(ps) & missing(mus) ){
    stop("One of mus or ps must be specified.", call. = FALSE)
  }
  ...
  if( sum( pmf ) < max( probs ) ){
    stop("The largest cumulative probability exceeds the range over which the distribution was evaluated.
